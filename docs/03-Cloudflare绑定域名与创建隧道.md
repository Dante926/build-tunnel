...
2. **navigate**→ **Domains** → **Overview** → **Add domain** → **connect a domain** → Continue
   ![alt text](/docs/assets/03.Bind-Domain-1.png)![alt text](/docs/assets/03.Bind-Domain-2.png)
...
2. 侧边导航栏找到 **Networking** → **Overview** → **Create a tunnel** → 输入任意 **Tunnel name** → 点击 **Create Tunnel** → 进入 Create Tunnel 详情页
   ![alt text](/docs/assets/03.Bind-Domain-5.png)
3. 选择 Docker 操作系统 → 复制 "Run tunnel with Docker" 指令：
   ![alt text](/docs/assets/03.Bind-Domain-6.png)

```sh
docker run cloudflare/cloudflared:latest tunnel --no-autoupdate run --token <TUNNEL_TOKEN>
```

4. 下一章会把它从命令行参数挪进 `.env` 文件，由 docker compose 注入。
