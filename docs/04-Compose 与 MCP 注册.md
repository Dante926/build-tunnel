...
上一章拿到了以 Docker 为安装环境的 `docker run` 指令。这一章改用 docker compose
统一管理 `cloudflared` 与 `mcpjungle`。

> **mcpjungle 是什么**：一个统一的 MCP 注册网关。它把多个上游 MCP server 收编成
> 单个 HTTP 端点供外部服务调用，并托管 stdio 类型的 MCP server 子进程。

## 1. 初始化目录与隧道

创建一个文件夹（按自己的风格命名，这里用 `tunnel/`），创建 `.env`：

```sh
TUNNEL_TOKEN = "<上一章得到的 TUNNEL_TOKEN>"
```

创建 `docker-compose.yaml`，先只放 cloudflared：
...
回到 Cloudflare → **Networking** → **Tunnel** → 域名，隧道状态应为 **Healthy**。
...
| `db_data` | 命名卷，存 mcpjungle 的注册表。`down` 不删，`down -v` 才会删 |
| `:rw` | 挂载可写。ChatGPT 因此能改写、删除该目录下任何文件；只读用途改成 `:ro` |

## 3. 注册 filesystem MCP

创建 `tunnel/mcp/filesystem.json`：
...
输出里能看到 `filesystem` 条目即为成功：

```json
[{"name":"filesystem","transport":"stdio","enabled":true,"command":"npx",
  "args":["-y","@modelcontextprotocol/server-filesystem","/host"],"session_mode":"stateless"}]
```

也可以访问 <http://127.0.0.1:8080> 在 dashboard 中查看注册记录。
